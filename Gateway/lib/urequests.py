...
class URLOpener:
    def __init__(self, url, method, params = {}, data = {}, headers = {}, cookies = {}, auth = (), timeout = 2):
        self.status_code = 0
        self.headers = {}
        self.text = ""
        self.url = url
        ...
        if auth and isinstance(auth, tuple) and len(auth) == 2:
            headers['Authorization'] = 'Basic %s' % (str(b64encode('%s:%s' % (auth[0], auth[1])))[2:-1])
        if scheme == 'http':
            ...
            s = ssl.wrap_socket(sock, keyfile=None, certfile=None, server_side=False, cert_reqs=ssl.CERT_NONE)
...
def b64encode(s, altchars=None):
    ...
    #    raise TypeError("expected bytes, not %s: %s" % (s.__class__.__name__, s))
    # Strip off the trailing newline
    encoded = binascii.b2a_base64(s)[0:-1]
